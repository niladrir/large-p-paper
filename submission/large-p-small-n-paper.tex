...
In classification problems, linear discriminant analysis (LDA) can be used to find a low-dimensional space where the groups are most separated. This corresponds to using the LDA index (\cite{JW02}) in projection pursuit:

Let $\mathbf{X_{ij}}$ be the $p$-dimensional vector of the $j$th observation in the $i$th class, $i = 1, \dots, g, j = 1, \dots, n_i$, $g$ is the number of classes, $n_i$ is the number of observations in class $i$, and $n = \sum_{i = 1}^g n_i$. Let $\bar{\mathbf{X}}_{i.} = \frac{1}{n_i} \sum_{j = 1}^{n_i} \mathbf{X}_{ij}$ be the $i$th class mean and $\bar{\mathbf{X}}_{..} = \frac{1}{n} \sum_{i = 1}^g \sum_{j = 1}^{n_i} \mathbf{X}_{ij}$ be the total mean. Let

$$\hbox{\bf{B}} = \sum_{i=1}^g n_i (\bar{\mathbf{X}}_{i.} - \bar{\mathbf{X}}_{..})(\bar{\mathbf{X}}_{i.} - \bar{\mathbf{X}}_{..})^T : \hbox{between-class sum of squares}$$

$$\hbox{\textbf{W}} = \sum_{i=1}^g \sum_{j=1}^{n_i} (\mathbf{X_{ij}} - \bar{\mathbf{X}}_{i.})(\mathbf{X_{ij}} - \bar{\mathbf{X}}_{i.})^T : \hbox{within-class sum of squares}$$

Then, the LDA PP index is 

$$
I_{LDA}(\hbox{A}) = \left\{ \begin{array}{cc}
...
Let $\mathbf{X}_{ij}^*$ be the standardized vector of $\mathbf{X_{ij}}$. Then 

$$\hbox{\bf{B}}^s = \sum_{i=1}^g n_i (\bar{\mathbf{X}^*}_{i.} - \bar{\mathbf{X^*}}_{..})(\bar{\mathbf{X^*}}_{i.} - \bar{\mathbf{X^*}}_{..})^T : \hbox{between-class sums of squares of the standardized data}$$

$$\hbox{\bf{W}}^s = \sum_{i=1}^g \sum_{j=1}^{n_i} ({\mathbf{X^*}_{ij}} - \bar{\mathbf{X}^*}_{i.})({\mathbf{X^*}_{ij}} - \bar{\mathbf{X^*}}_{i.})^T : \hbox{within-class sums of squares of the standardized data }$$

where $\bar{\mathbf{X^*}}_{i.}$ is the $i$th class mean of the standardized data and $\bar{\mathbf{X^*}}_{..}$ is the total mean of the standardized data. Then the PDA index is defined as
...
where \textbf{A} is an orthonormal projection onto a $k$-dimensional space and $\lambda \in [0, 1)$ is a predetermined parameter.
...
The goal of the experiment is to determine how well random noise is distinguishable from real separation. The real separation is introduced in the data in one or two variables. Data is simulated for different values of dimension $p$ ( = 20, 40, 60, 80, 100) keeping the sample size $n$ fixed (at 30) and for the presence or absence of real separation (separation = Yes or No) . Also both one and two dimensional projections are obtained. The choice of range of dimension $p$ is explained in Section \ref{sec:theory}. Data is simulated from a standard normal distribution. Data sets are generated with parameter values given in Table \ref{freq}. Three replicates at each level were generated. These produced 60 different ``observed data sets", and thus, 60 different lineups. 

\begin{table}[htbp]
\begin{center}
\caption{Values of parameters considered for the experiment. The choice of the dimensions is explained in Section \ref{sec:theory}.}
...
To bring in the real separation in the data, each element of the $p$-th variable $\mathbf{X}_p$ which belongs to group 1 is subtracted by 3 and 3 is added to each element of $\mathbf{X}_p$ which belongs to group 2. So, 
$$
...
Hence two sets of data are obtained, each with $p$ dimensions with $n = 30$ observations in each dimension divided into two groups. One of the datasets is purely noise and the other has some real separation in the form of the $p$-th dimension. On each of these datasets of $p$ dimension, a projection pursuit optimization with a PDA index is performed to obtain the $d = 1$ dimensional projections.  

The above procedure is repeated to obtain the $d = 2$ dimensional projections. But instead of labeling the 15 observations as group 1 and group 2, the first 10 observations is assigned as group 1, the second 10 observations as group 2 and the last 10 as group 3. As before a $30 \times (p + 1)$ matrix, say, $ \mathbf{X}$, is obtained where the first 10 observations are from group 1, the second 10 observations are from group 2 and the last 10 observations are from group 3.  So, collectively,  $\mathbf{X}$ can be written as
...
The one dimensional projections obtained from the projection pursuit with the PDA index are plotted on the horizontal axis against fixed value (1.5 in this case) on the vertical axis colored by the group variable to obtain the observed data plot. The limits of the vertical axis are adjusted so that the points form circular clusters instead of bands of points. The 19 null plots in the lineup are obtained by  permuting the group variable in order to break any dependence between the group variable and the other variables and the one dimensional projections are obtained and plotted in the same way. The observed data plot is placed randomly among the 19 null plots to obtain the lineup. For each plot in the lineup, the mean of the projections for each group is calculated and the group with the lower mean is considered as group 1 and the other as group 2. This method helps in maintaining the same orientation of the groups in each plot of the lineup. The same symbol is used to represent both the groups so that the different symbols do not confound the analysis. A statistic WBratio which is the ratio of the average distance within clusters to the average distance between clusters (\cite{hennig:2010}) is calculated for each of the 20 plots in the lineup. 

The above procedure is repeated for the one dimensional projections for data with one dimension of real separation. Figure \ref{fig:test_category_1d} gives the lineup  for the one dimensional projections with $p = 20$ dimensions where 19 dimensions are purely noise data and 1 dimension has real separation. This is an example of a lineup that is used in the experiment. 
...
Similar procedure is followed to obtain the lineup  for two dimensional projections for purely noise and data with two dimensions of real separation. But, in this case, the first two projections are plotted on the horizontal and vertical axis respectively colored by the group variable and using the same symbol for the groups to get a scatterplot to obtain the observed data plot. For the null plots, the group variable is permuted and the projection pursuit is performed on the permuted dataset and plotted in the same way. To account for the occasional convergence problem with the optimization, 30 null plots are generated. The 19 null plots which have the smallest Wilk's $\lambda$  (\cite{JW02}) values are used for the lineup. The observed data is placed randomly among the 19 null plots to obtain the lineup. Figure \ref{fig:test_category} gives the lineup  for the two dimensional projections for $p =100$ dimensions where all the 100 dimensions are purely noise data. WBratio are calculated for each of the 20 plots in the lineup. 
...
Subjects  for the experiment are recruited through Amazon Mechanical Turk  \citep{turk}.  \cite{turk} is a platform to get feedback on this kind of experiments. Each subject is shown a sequence of 10 lineups. Subjects also had the freedom of answering more than 10 lineups. They are asked to identify the plot which has the most separation between the colored groups. Their response is recorded along with a reason for their choice of the plot and the level of confidence they have in their decision.  Gender, age, educational qualification and location of each subject are also noted. In total, 1137 lineups are evaluated by 103 subjects from different locations.
...
Amazon Mechanical turk subjects are paid for their responses. Since the process is not monitored manually, some subjects do not make an honest effort to find the observed data plot. To counter this problem, each subject was given a very easy lineup (a lineup with $p = 10$ dimensions with some real separation). The subjects who failed to give a correct response to this lineup were removed from the study. If the response in this lineup is correct, we remove the response of this lineup but keep all the other responses.
...
It has been observed that even if the data is purely noise, the groups separate out as the number of dimensions increases for a fixed sample size. Figure \ref{dist_1d} illustrates that in greater details. The same experiment was done here with purely noise data with 30 observations divided into 2 groups with 15 observations in each. The number of dimensions are increased gradually and the projection pursuit is performed on the above data and the one dimensional projections are plotted as before. It can be noticed that as the number of dimensions increases for fixed sample size $n$, it can be noticed that the colors starts separating out and the clusters are formed. Figure \ref{dist_1d} shows the one dimensional projections for four different values of dimension ($p = 2, 20, 50, 100$).
...
%	\scalebox{0.25}{\includegraphics{plot_2d_100.pdf}}
       \caption{Plots showing one dimensional projections for 4 different number of dimensions $p=3$, $p=20$, $p=50$ and $p=100$. Note that the groups look more and more separated as the number of dimensions increases when actually there is no real separation and the data is purely noise.  }
       \label{dist_1d}
\end{figure*}
