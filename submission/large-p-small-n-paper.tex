...
%\begin{figurehere}
   \centering
       ...
       \caption{LD1 versus LD2 from an LDA on a randomly selected subset of 40 significantly different oligos : F, Foundress; G, gyne; Q, queen and W, worker. It can be noticed that the groups F and G are separated. This figure is regenerated to match Figure 2 in \cite{toth:2010}. }
     \label{oligo}
%\end{figurehere}
\end{figure*}  
...
To check for the optimization procedure one dimension of real separation is considered in the data. To bring in the real separation, we adjust the mean of $X_{ij}$. We subtract $C$ from the mean of the first 15 observations (group 1) and add $C$ to the mean of the last 15 observations (group 2). So we define
