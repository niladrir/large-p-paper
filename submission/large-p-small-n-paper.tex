...
%\begin{figurehere}
   \centering
       ...
       \caption{Plot showing the distribution of the sum of absolute difference of means for data with and without separation for different dimensions. The distributions of data with real separation ($V$) and purely noise data ($U$) are shown in blue and red respectively with the dark blue line showing the 5th percentile of $V$. The dark blue area shows the area of $U$ which is greater than the 5th percentile of $V$. The dark blue region increases as dimension ($p$) increases indicating that the probability of obtaining the sum of absolute difference for data with purely noise similar to that for data with real separation increases with $p$. }
     \label{fig:dimen}
%\end{figurehere}
\end{figure}
...
\begin{figure}[htbp]
\centering
\mbox{\subfigure[$T_1(\bf{Y})$]{\includegraphics[width=2.5in]{test_statistic_1.pdf}}\quad
\subfigure[$T_2(\bf{Y})$]{\includegraphics[width=2.5in]{test_statistic_2.pdf} }}
...
The hope is that visual inference can be used to improve the understanding of the emptiness of space in HDLSS data - that a 1D or 2D projection of HDLSS data, e.g. as provided by LDA, will likely look separated purely due to this emptiness. \cite{ripley:1996} ({\color{red}equation No}) quantifies the probability of seeing separation purely by chance, given a fixed sample size. Figure \ref{dist_1d} illustrates the phenomenon with 1D projections of samples of size $n = 30$, for different $p$. When $p = 2$, $\hbox{P(separation}|n = 30, p = 2) = 0$ and it reaches 1 when $p = 25$. For data of the size of the wasps, $n = 50$, the probability of obtaining separation with 2 groups is 1 when $p = 38$, so it would be much less than this if we could calculate the probability for 4 groups. Visual inference makes this clearer.


\begin{figure}[htbp]
\centering
\mbox{\subfigure[P(sep$|n,p=2$) = 0.00]{\includegraphics[width=1.52in]{noise-2.pdf}}\quad
\subfigure[P(sep$|n,p=12$) = 0.23]{\includegraphics[width=1.52in]{noise-12.pdf} }\quad
\subfigure[P(sep$|n,p=20$) = 0.99]{\includegraphics[width=1.52in]{noise-20.pdf} }\quad
\subfigure[P(sep$|n,p=28$) = 1.00]{\includegraphics[width=1.52in]{noise-28.pdf} }}
...
Let us denote ${\bf{X}}_{ij}$ as the $j$-th observation in the $i$-th group where $j = 1, \dots, n_i$. Essentially in our case there are only $i = 2$ groups. ${\bf{X}}_{ij}$s are purely noise obtained from a standard normal distribution. The difference between the means of the purely noise data for the two groups, group 1 and group 2 is given by $\bar{X}_{1.} - \bar{X}_{2.}$ and $$\bar{X}_{1.} - \bar{X}_{2.} \sim \hbox{Normal}(0, 1/n_1 + 1/n_2)$$ where $n_1 = n_2 = 15$. So we have $$\bar{X}_{1.} - \bar{X}_{2.} \sim \hbox{Normal}(0, 2/15)$$ Let us define 
$$U = |\bar{X} _{1.} - \bar{X}_{2.}|$$ 
where $U \sim \hbox{Half Normal}$ with scale parameter $ \sigma = \sqrt{1/n_1 + 1/n_2} = \sqrt{2/15}$. \\

The expectation and the variance of $U$ can be calculated: 
$$E(U ) = \sigma \sqrt{2/\pi}$$
$$Var(U) = \sigma^2 (1 - 2/\pi)$$

Let us consider $p$ dimensions of purely noise data, each dimension being divided into $i = 2$ groups with $n_i = 15$ observations. Let us define $$U_m = |\bar{X}_{m1.} - \bar{X}_{m2.}|$$ where ${\bf{X}} _{mij}$ is the $j$-th observation in the $i$-group for the $m$-th dimension. The sum of the absolute difference between the means is obtained for all the $p$ dimensions. So, define $$U  = \sum_{m=1}^p {\bf{U}} _m = \sum_{m=1}^p |\bar{{\bf{X}}}_{m1.} - \bar{{\bf{X}}}_{m2.}| $$ \\

Assuming independence among the dimensions of purely noise data,

$$E(U) = p \sigma \sqrt{2/\pi}$$
$$Var(U) = p \sigma^2 (1 - 2/\pi)$$

To check for the optimization procedure one dimension of real separation is considered in the data. To bring in the real separation, we adjust the mean of ${\bf{X}}_{ij}$. We subtract any value $c$ from the mean of the first 15 observations (group 1) and add $c$ to the mean of the last 15 observations (group 2). So we define
$${\bf{X}}_{ij}^* \sim \hbox{Normal}(- c, 1) \qquad \hbox{for} \qquad j = 1, \dots, n_1$$
$${\bf{X}}_{ij}^* \sim \hbox{Normal}(c, 1) \qquad \hbox{for} \qquad j = 1, \dots, n_2$$ 
Hence we have $$\bar{X}_{1.}^* - \bar{X}_{2.}^* \sim \hbox{Normal}(2c, 1/n_1 + 1/n_2)$$ where $n_1 = n_2 = 15$. Let us define $${U}^*  = |\bar{X}_{1.}^* - \bar{X}_{2.}^*|$$ where ${U}^* \sim \hbox{Folded Normal Distribution}$ with scale parameter $ \sigma = \sqrt{1/n_1 + 1/n_2} = \sqrt{2/15}$.  \\

The expectation and the variance of $Y^*$ can be calculated :

$$E(U^*) = \sigma \sqrt{2/\pi} \exp(- 2c^2/\sigma^2) + 2c[1 - \Phi(-2c/\sigma)]$$
$$Var(U^*) = 4c^2 + \sigma^2 - (E(U^*))^2$$


Now the means of ${\bf{X}}_{mij}$s are adjusted in the $p$-th dimension. So we define $V$ as the sum of the absolute differences of the mean with one dimension of real separation as $$V = \sum_{m=1}^{p-1} |\bar{{\bf{X}}}_{m1.} - \bar{{\bf{X}}}_{m2.}| + U_p$$ where $U_p$ follows a Folded Normal Distribution.
...
$$Var(V) = (p - 1)\sigma^2 (1 - 2/\pi) + 4c^2 + \sigma^2 - \left( \sigma \sqrt{2/\pi} \exp(- 2c^2/\sigma^2) + 2c[1 - \Phi(-2c/\sigma)]\right)^2$$
...
$$Var(V) = (p - 1)\sigma^2 (1 - 2/\pi) + \sigma^2$$
As the value of $p$ increases for a fixed $n$, the spread of $U$ increases with a factor of the dimension $p$ and the spread of $V$ increases as well. The means of both $U$ and $V$ also increase with a factor of $p$ but the expected value of the difference between $U$ and $V$ stays constant and is independent of the number of dimensions ($p$). 
%\begin{equation}
$$E (V - U) = (p - 1) \sigma \sqrt{2/\pi} + 6 - p \sigma \sqrt{2/\pi} = 6 - \sigma \sqrt{2/\pi}$$
%\end{equation}


 Figure \ref{fig:dimen} shows the phenomenon described above. In Figure \ref{fig:dimen} notice that as the number of dimensions ($p$) increases from 20 to 100, the dark blue region of the distribution of $U$ which is greater than the 5th percentile of $V$ increases as the variability in $U$ and $V$ increases as a factor of the dimension ($p$).
...
The next step is to find the number of dimension for a specific value of the common region between the two distributions. To allow some error, the value of the number of dimension $p$ is considered for which $$\hbox{P}[U > V_{\alpha}] = \delta$$ where $V_{\alpha}$ is the $\alpha$-th percentile of $V$.  So for a given value of $\delta$,  the value of $p$ is calculated for which the above condition holds true for $\alpha = 0.05$.
