...
Let us consider $p$ dimensions of data with random noise with $i = 2$ groups with 15 observations in each group. Let us define $${\blU}_m = |\bar{{\blX}}_{m1.} - \bar{{\blX}}_{m2.}|$$ where ${\blX} _{mij}$ is the $j$-th observation in the $i$-group for the $m$-th dimension. The sum of the absolute difference between the means is obtained for all the $p$ dimensions. So, define $$\mbox{U}  = \sum_{m=1}^p {\blU} _m = \sum_{m=1}^p |\bar{{\blX}}_{m1.} - \bar{{\blX}}_{m2.}| $$ \\
...
We also consider one dimension of real separation in the data. To bring in the real separation, we adjust the means of ${\blX}_{ij}$. We subtract any value $c$ from the mean of the first 15 observations (group 1) and add $c$ to the mean of the last 15 observations (group 2). So we define
$${\blY}_{ij}^* \sim \hbox{Normal}(- c, 1) \qquad \hbox{for} \qquad j = 1, \dots, 15, i = 1$$
$${\blY}_{ij}^* \sim \hbox{Normal}(c, 1) \qquad \hbox{for} \qquad j = 1, \dots, 15, i = 2$$ 
Hence $$\bar{\mbox{Y}}_{1.}^* - \bar{\mbox{Y}}_{2.}^* \sim \hbox{Normal}(2c, 2/15)$$ Define $$\mbox{Y}^*  = |\bar{\mbox{Y}}_{1.}^* - \bar{\mbox{Y}}_{2.}^*|$$ where ${\mbox{Y}}^* \sim \hbox{Folded Normal Distribution}$ with scale parameter $ \sigma = \sqrt{2/15}$.  \\

The expectation and the variance of $\mbox{Y}^*$ can be calculated :

$$E(\mbox{Y}^*) = \sigma \sqrt{2/\pi} \exp(- 2c^2/\sigma^2) + 2c[1 - \Phi(-2c/\sigma)]$$
$$Var(\mbox{Y}^*) = 4c^2 + \sigma^2 - (E(\mbox{Y}^*))^2$$


The means of ${\blX}_{mij}$s are adjusted in the $p$-th dimension. Define $\mbox{V}$ as the sum of the absolute differences of the mean with one dimension of real separation as $$\mbox{V} = \sum_{m=1}^{p-1} |\bar{{\blX}}_{m1.} - \bar{{\blX}}_{m2.}| + {\blY}^*_p$$ where ${\bf{Y}}_p^*$ follows a Folded Normal Distribution.
