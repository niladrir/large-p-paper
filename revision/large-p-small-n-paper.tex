...
We were skeptical about the convergence of the optimization procedure of projection pursuit in \texttt{tourr} package \citep{tourr:2011} in \texttt{R} \citep{r}. The projection pursuit finds low dimensional projections of high dimensional data. The separation between groups, even for random noise data seemed identifiable. Visual inference provides a way to test this. This technique can be used to test the convergence algorithm in any other method where the optimization procedure is involved like multidimensional scaling, PCA, independent component  analysis (ICA) and local linear embeddings.
...
For example, suppose we have data on the concentration of a metal in $mg/kg$ for two sites A and B.
...
\section{Amazon Turk Experiments}
