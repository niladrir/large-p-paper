...
An experiment is designed to study the ability of human observers to detect the effect of one or two dimensions of real separation in $p$ dimensions of noise. Data is simulated for different values of $p$  ( = 20, 40, 60, 80, 100).  So two groups of $p$ dimensions of data with sample size $n = 15$ were generated from Normal(0 ,1).  The data from the first group is labeled as class 1 and the data from the second group as class 2. So a 30 $\times$ ($p$ + 1) matrix , say, $X$, is obtained where the first 15 observations are from class 1 and the last 15 observations are from class 2.  So X can be written as
...
To bring in the real separation in the noise data, 3 is subtracted from the elements of the $p$-th variable $X_p$ which belongs to class 1 and 3 is added to the elements of the same variable which belongs to class 2. So, 
$$
X_p = \left\{ \begin{array}{rl}
 ...
 X_p + 3 &\mbox{ if $X_p \in$ class 2}
       \end{array} \right.
$$
This is done so that the mean of the observations in the two classes in the $p$-th dimension is separated by 6 units. This modified $X$ matrix works as the data with one dimension of real separation. 

Hence two sets of data is obtained, each with $p$ dimensions with 30 observations in each dimension divided into two classes. One of the datasets is pure noise and the other has some real separation in the form of the $p$-th dimension. On each of these datasets of $p$ dimension, a projection pursuit optimization with a PDA index is performed to obtain the $d = 1$ dimensional projections.  

The above procedure is repeated to obtain the $d = 2$ dimensional projections as well. But instead of labeling the 15 observations as class 1 and class 2, the first 10 observations is assigned as class 1, the second 10 observations as class 2 and the last 10 as class 3. Once again a 30 $\times$ ($p$ + 1) matrix , say, $X$, is obtained where the first 10 observations are from class 1, the second 10 observations are from class 2 and the last 10 observations are from class 3.  So X can be written as
...
As before to bring in the real separation, the means of the 3 classes are adjusted in the last two dimensions i.e. $X_{p-1}$ and $X_p$. The adjustment is done in the following way:
$$
(X_{p-1}, X_p) = \left\{ \begin{array}{rl}
 ...
 (X_{p-1} , X_p + \sqrt{27}) &\mbox{ if $(X_{p-1}, X_p) \in$ class 3}
       \end{array} \right.
$$
If we plot $X_{p-1}$ versus $X_p$ in a scatterplot, the points cluster along the vertices of an equilateral triangle of side 6. $X$ works here as the data with real separation in the last two dimensions . In this manner the data with 2 dimensions of real separation divided into 3 classes is obtained. 
So for obtaining the two dimensional projections two sets of data are obtained - one with random noise and the other with two dimensions of real separation. Again on each of the above datasets a projection pursuit with a PDA index is performed to obtain the $d = 2$ dimensional projections. 
...
To obtain a lineup with $m = 20$ plots for one dimensional projections for random noise, the one dimensional projections obtained from the projection pursuit with the PDA index is plotted on the horizontal axis and a fixed value (1.5 in this case) is plotted on the vertical axis colored by the class variable. This plot is called the ``observed plot" as it is obtained from the observed dataset. The limits of the vertical axis are adjusted so that the points form circular clusters instead of bands of points. The 19 null plots in the lineup were obtained by  permuting the class variable so that breaking any dependency between the class variable and the other variables. Then the ``observed plot" is placed randomly among the 19 null plots to obtain one lineup. Two different statistics Wilk's $\lambda$ (\cite{JW02})  and the within sum of squares to between sum of squares ratio (BW ratio) are calculated for each of the 20 plots in the lineup. 
...
Similar procedure was followed to obtain the lineup plot for two dimensional projections for random noise and data with two dimensions of real separation. But in this case the first two projections are plotted on the horizontal and vertical axis respectively colored by the class variable to get a scatterplot. This again gives the ``observed plot". To obtain the null plots, the class variable is permuted and the projection pursuit is performed on the permuted dataset. The ``observed data" is placed randomly among the 19 null plots to obtain a lineup of $m = 20$ plots. Figure \ref{fig:test_category} gives the lineup plot for the two dimensional projections for $p =100$ dimensions where all the 100 dimensions are pure noise data. As before the Wilk's $\lambda$ (\cite{JW02})  and the within sum of squares to between sum of squares ratio (BW ratio) are calculated for each of the 20 plots in the lineup. 
...
To decide on the different dimensions, the distribution of the absolute difference of the means of random noise divided into two classes is looked at. In this case one dimension of random noise is considered which is divided into two class. The means of the observations in each class is calculated. Since we are interested in the projections, the absolute differences of the means is considered.
...
%\begin{figurehere}
   \centering
       ...
       \caption{Plot showing the distribution of the sum of absolute difference of means for data with pure noise ($Y$) and real separation ($Z$) for different values of the dimension. The distribution of $Y$ and $Z$ are colored in red and blue respectively with the blue line showing the 5th percentile of $Z$. The blue area shows the area of $Y$ which is greater than the 5th percentile of $Z$. We can notice that the blue area increases as we increase the number of dimensions. This indicates that the probability of obtaining the sum of absolute difference for data with pure noise similar to the difference for data with real separation increases with $p$. }
     \label{fig:dimen}
%\end{figurehere}
\end{figure}

 Figure \ref{fig:dimen} shows the phenomenon described above. In Figure \ref{fig:dimen} notice that as the number of dimensions ($p$) increases from 20 to 100, the region of the distribution of $Y$ which is greater than the 5th percentile of $Z$ increases as the variability in $Y$ and $Z$ increases as a factor of the dimension ($p$).
...
\begin{table}[htbp]
\begin{center}
\caption{Numerical summaries of dimension $p$ for each value of $\delta$. Notice that with the increase in the common region $\delta$, the median number of dimensions required to obtain the common region also increases. This also means for a large value of $p$ the difference between the groups for data with noise is close to the difference between the groups for data with real separation. }
\begin{tabular}{cccp{3cm}}
  \hline
  \hline
...
It would be interesting to look at the proportion of participants identifying the correct plot in the presence of real separation versus pure noise, for 1d projections versus 2d projections and for the different dimensions.
...
and $x_{lk}$ is the dimension of the $l$-th lineup for the $k$-th participant. $\gamma$ is the corresponding slope parameter and $g(.)$ denotes the logit link function $g(\pi)$ = log($\pi$) - log(1 - $\pi$); $0 \leq \pi \leq 1$. The logit link function is inverted to estimate the lineup specific probability of successful evaluation as 
\begin{equation}
\hat{p}_{lk} = g^{-1}(\hat{\theta} + \hat{\alpha}_{lk} + \hat{\beta}_{lk} + \hat{\gamma} x_{lk}) \label{invert}
\end{equation}

Model \ref{fixed} is fitted to the data and the probabilities of successful evaluation for each lineup is obtained.  The line in Figure \ref{suc-rate-glm} shows the estimated probability of successful evaluation for the different levels of the parameters which the points gives the observed success rate. Notice that as the number of dimensions $p$ in the lineups with real separation increases, the probability of success in picking the true plot decreases. This is understandable since as the number of dimensions of noise increases keeping the number of real separation fixed to one or two, the distance between the groups in the actual plot get closer to the distance between the groups in a null plot making it difficult for the participants to pick the actual plot. Table \ref{params} shows the estimates of the parameters, the standard errors and the corresponding $p$-values.

\begin{table}[ht]
\begin{center}
\caption{Table showing the estimate, the standard error and the $p$-value of the parameters used in Model \ref{fixed}. Notice that the covariates dimension and noise are highly significant while projection is not significant at 5\% level of significance.}
\vspace{0.15cm}
\begin{tabular}{r|ccc}
\hline
  \hline
...
Intercept  & 1.953 & 0.253  & 0.000 \\ 
  ...
  purely noise data & $-$3.750 & 0.298  & 0.000 \\ 
  2d projections & $-$0.079 & 0.176  & 0.653 \\ 
   \hline
\end{tabular}
\label{params}
\end{center}
...
In Table \ref{params} observe that the $p$-values corresponding to dimension and presence of real separation is very highly significant. But the $p$-value corresponding to the projection is not significant which is suggested in the previous section as well.

The estimated models for the different treatment levels are provided. 

$$
g(\hat{p}_{lk}) = \left\{ \begin{array}{rl}
 1.953 - 0.026 x_{lk} &\mbox{for data with real separation and 1D projections} \\
 -1.797 - 0.026 x_{lk} &\mbox{for data with purely noise and 1D projections} \\
  1.874 - 0.026 x_{lk} & \mbox{for data with real separation and 2D projections} \\
  -1.876 - 0.026 x_{lk} &\mbox{for data with purely noise and 2D projections} \\
       \end{array} \right.
$$

The fitted models for the different treatment levels suggest that keeping the covariate projection constant at one particular level, there is a difference between the fitted models for data with real separation and data with purely noise. On the other hand, keeping the presence of noise variable constant at a particular level, there is not much of a difference between 1D projections and 2D projections in the fitted models.  
...
%\begin{figurehere}
   \centering
       \scalebox{0.6}{\includegraphics{subjectwise-glm.pdf}}
%        \scalebox{0.80}{\includegraphics{result-noise.pdf}}
...
In classical inference the test statistic can be compared with an infinite number of possible values from the sampling distribution under the null hypothesis. But in visual inference the participants can compare the test statistic (the observed plot) with only a finite number of null plots which makes the null plots an important factor in decision making. It is mentioned in Section \ref{effects} that the probability of correct evaluation decreases for dimension $p = 80$ and increases for dimension $p = 100$ which is opposite to what we should expect. To investigate this the difference between the minimum WBratio of the null plots and the WBratio for the observed plot for each lineup is considered. Since the values of WBratio vary for each lineup depending on the presence or absence of real separation, the difference is scaled by dividing the difference by the WBratio of the observed plot of the lineup. To put the differences on a logarithmic scale, the differences are adjusted by adding a constant term. Figure \ref{null} shows the relationship between the logarithm of the adjusted difference and the probability of successful evaluation.  

\begin{figure*}[hbtp]
%\begin{figurehere}
   \centering
       \scalebox{0.6}{\includegraphics{suc-diff-wbratio-glm.pdf}}
       \caption{Proportion of successful evaluation is plotted against the adjusted relative difference between the minimum WBratio of the null plots and the WBratio of the observed plot on a logarithmic scale for each lineup. The vertical line gives the 0 difference line on the log scale. The points left to the zero line indicates a difficult lineup in the sense that at least one of the null plots had a lower WBratio value than the observed plot. The blue line shows a generalized linear regression model which indicates that there is a positive effect of the differences on the probability of successful evaluations.}
       \label{null}
\end{figure*}
...
%\begin{figurehere}
   \centering
       ...
       \caption{Lineup Plot showing LD1 versus LD2 from an LDA on a randomly selected subset of 40 significantly different oligos. F, Foundress; G, gyne; Q, queen and W, worker. The actual data plot is placed randomly among the 19 null plots. Which plot shows the most separation between the 4 groups? }
       \label{toth_lineup}
\end{figure*} 
...
The probability of successful evaluation for the wasp data and permuted data is shown in Table \ref{wasp}. As we can see, none of the participants were able to identify the actual plot in the wasp data. But around 10\% of the participants were able to identify the actual plot when we used the permuted data to generate the lineup.
...
\begin{table}[ht]
\begin{center}
\caption{Table showing the proportion of correct response for wasp data and permuted data with 95\% adjusted Wald intervals and the number of attempts. Notice that the participants were able to identify the observed plot when the data is permuted from the wasp data.}
\vspace{0.15cm}
\begin{tabular}{r|ccc}
\hline
  \hline
 Data & Proportion of successful evaluations & 95\% Adjusted Wald Intervals  & Total attempts\\ 
  \hline
Wasp Data & 0.0000 & (0,  0.0408  )  & 65\\ 
Permuted Data & 0.0980 & (0.0066, 0.1895) & 51 \\ 
