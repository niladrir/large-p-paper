...
\cite{buja:2009}, following from Gelman [2004], proposed two protocols that allow the testing of discoveries made from statistical graphics. Visual Inference can be used to test a number of hypothesis for which a classical test is unknown till date. Unlike classical hypothesis testing, the test statistic in visual inference is not a real number, but a plot that is appropriately chosen to describe the parameter of interest . When the alternative hypothesis is true, it is expected that the plot of the observed data, the test statistic, will have visible feature(s) consistent with the null hypothesis and human subjects will be able to identify the plot of the observed data as different from all the null plots. \cite{buja:2009} talks about this protocol and also gives several example of different situations where this method can be applied. Later in 2012, \cite{mahbub:2012} does a comparative study of the power of the classical test and the visual test and shows that visual test does a reasonable job in a classical situation. But in situations where the classical test is unknown till date, the visual test still maintains a good power. 
...
An experiment is designed to study the ability of human observers to detect the effect of one dimension of real separation in $p$ dimensions of noise. Data is simulated for different values of $p$  ( = 20, 40, 60, 80, 100).  So $p$ dimension of data with sample size $n = 30$ were generated from Normal(0 ,1). We bring in a class variable where the first 15 observations came from class 1 and the last 15 observations came from class 2.  We subtracted 3 from the first 15 observation i.e class 1 and added 3 to the last 15 observation i.e. class 2 to the $p$-th dimension to bring in the real separation.  We repeated the above procedure for all the different values of $p$.  To compare the data with real separation we simulate $p$ dimensional data from a N(0, 1) distribution. This data is complete noise and has no real separation.  For both these types of data (real and noise) a projection pursuit optimization was applied to obtain the one dimensional projections. To obtain the null plots, we permute the class variable breaking any dependence between the class variable and the other variables. Then the plot of the observed data is placed randomly among the null plots to obtain one lineup. 

We repeat the same procedure to obtain the two dimensional projections as well. But here we assign the first 10 observations as class 1, the second 10 observations as class 2 and the last 10 as class 3. For bringing in the real separation we adjust the means of the three classes such that the three means of the three classes represents the three vertices of an equilateral triangle. As before, we applied the projection pursuit to obtain the two dimensional projections.
  
Data sets with different combinations of $n$, $p$ and presence of noise were generated with frequencies shown in Table \ref{freq}. Three replicates of each level were generated.These produced 60 different ``observed data sets''. 
...
\end{table}
 ...
 \cite{turk} is an amazing platform to get feedback on this kind of experiments. People were recruited from \cite{turk} and each participant were shown 10 lineups. People were asked to identify the plot which has the largest separation among the groups. One of the lineups were simulated from a 10- dimensional data with real separation. Each participant received one of these lineups and this was used as a test plot. We recorded the feedback from the participants on each lineup. We also recorded other demographic information like age, gender and educational qualifications.
...
%\begin{figurehere}
   \centering
       \scalebox{0.7}{\includegraphics{subjectwise-glm.pdf}}
%        \scalebox{0.80}{\includegraphics{result-noise.pdf}}
