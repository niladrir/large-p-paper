...
As we increase the value of $p$ for a fixed $n$, the distribution of $Y$ goes closer and closer to the distribution of $Z$ since every time we increase the number of dimension, a value is added to the mean of the distributions of $Y$ and $Z$. Figure \ref{fig:dimen} shows the phenomenon. 
...
%\begin{figurehere}
   \centering
       \scalebox{0.60}{\includegraphics{sum-noise-real.pdf}}
...
In Figure \ref{fig:dimen} we can see as we increase the number of dimensions from 20 to 100, the common region between the distribution of $Y$ and the distribution of $Z$ increases indicating that for a large dimension, the sum of the absolute differences of the means for random noise would be close to the sum of the absolute differences for the means with only one dimension of real separation. 
