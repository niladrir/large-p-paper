...
%\begin{figurehere}
   \centering
       \scalebox{0.6}{\includegraphics{suc-diff-wbratio-dot.pdf}}
       \caption{Proportion of successful evaluation is plotted against the adjusted relative difference between the minimum WBratio of the null plots and the WBratio of the observed plot on a logarithmic scale for each lineup. The line gives the 0 difference line on the log scale. The points left to the zero line indicates a difficult lineup in the sense that at least one of the null plots had a lower WBratio value than the observed plot. The blue line shows a linear regression model which indicates that there may be a positive effect of the differences on the probability of successful evaluations.}
       \label{null}
\end{figure*}
